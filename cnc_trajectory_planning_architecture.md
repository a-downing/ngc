...
Rolling publication may split geometry only at the midpoint of a source line strictly longer than `6P`, where the split lies inside that line's exact retained middle. A line of length `6P` or less belongs to the short-entity reconstruction region and is not a legal rolling anchor. This keeps Preview and timed execution on the same cluster partition instead of pinning separately reconstructed prefix and suffix curves to an original G-code midpoint. The production exact-correction ceiling is 32 passes so an all-short retained suffix can remain one geometry-consistent horizon while still requiring final proof.
