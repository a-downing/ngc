...
Planning diagnostics are NRT-only and report command, `PlanChunk`, G64, and individually planned continuous-command exact-stop counts, lookahead and span high-water marks, planned duration, first/last/maximum continuous-horizon latency, total continuous-horizon computation time, total planning computation time, and rolling-boundary candidate failures. Timed simulation exposes them through `SimulationSnapshot`; do not add them to `MotionBackend`. The optional oracle exporter `--rolling` mode prints every horizon's duration and computation time plus the aggregate comparison; `--rolling-only` skips the full-horizon oracle export for faster horizon-policy profiling. Its explicit `--effort` profiles may raise NRT reachability sweeps, velocity refinements, and resource budgets for offline convergence experiments; they must not change production defaults.
...
Any maximal run of entities with arc length at most `6P` that is bounded on both sides by entities longer than `6P` is replaced by one direct open-clamped cubic B-spline. Its first and last three controls are the normal curvature-continuous controls sampled `3P` into the bounding long entities. If the run's total source length is less than `6P`, it contributes no interior controls. Otherwise its composite line/arc path is sampled at evenly distributed arc-length positions, targeting approximately one control per `P` while never using more controls than the original entity count. The current bounded conditioner leaves the first and last three controls fixed, fairs only the free control polygon within `0.2P`, and accepts the candidate only when an independent analytic measurement reduces the sampled peak arc-length curvature derivative; it is not a fit-point solve and does not establish curvature-derivative continuity. Execution applies one local timing limit to at most three consecutive polynomial knot spans so a localized curvature-derivative limit does not throttle the complete spline. Programmed feed changes add timing-only boundaries mapped proportionally through the collapsed source path; each side retains its own entity feed and their shared station observes the lower adjacent cap. Different incoming and outgoing entities may and normally do contribute different local scales to an ordinary junction.

Every displayed spline is an open-clamped uniform degree-three B-spline. An ordinary six-control junction uses knot vector `[0,0,0,0,1,2,3,3,3,3]`; an `N`-control short-entity spline uses the corresponding uniform knot range through `N-3`. Do not evaluate the controls as a single Bezier curve, expose knot-inserted Bezier controls, or restore tolerance fitting, fit-point solving, or recursive cluster fitting.

Curvature-derivative investigation is NRT development work. The optional oracle `--trace-slow` capture writes exact source primitives, spline controls, timing-piece boundaries, and a separate piece-timing CSV. `ngc_spline_geometry_analyzer` measures analytic arc-length curvature derivatives, produces constant-feed acceleration/jerk profiles, and contains bounded experimental cubic optimizers. `ngc_quintic_spline_analyzer` reconstructs a simple-knot degree-five spline from the same captured primitive chain, constrains endpoint position through curvature derivative, optimizes its free controls under a sampled `0.2P` primitive-deviation bound, and emits CSV/SVG comparisons. These tools are diagnostic only: their sampled deviation and simplified standalone arc reconstruction are not executable geometry proofs. Do not move an experimental cubic or quintic into preview or execution until both consumers share the exact construction, endpoint-exact arc reference, ordered geometry verification, feed mapping, activation ownership, capacity proof, and complete regression evidence.
...
1. Extend the current coupled-limit-aware `(v,a)` station optimization into complete forward/backward reachable-set propagation whose state includes the emitted-polynomial correction consequences, then add adaptive timing stations. The v3 jerk-aware oracle profile shows dense boundary-speed loss and `1001.ngc` acceleration-carry loss, but naive local-veto relaxation, a scalar global acceleration lattice, and simple curved-piece midpoint splitting all worsened or failed final verification and were removed. Preserve the stable dense and `1001.ngc` durations.
2. Add numerically identical arc-interior anchors without weakening the strict PVA boundary proof; retained-line anchors currently provide incremental publication for mixed and line-segmented paths.
3. Reduce rolling suffix-probe CPU cost and implement bounded resumable feed-hold latency while preserving nonzero motion across rolling horizon boundaries. Candidate-solve, geometry-proof, and staged-span work already have explicit guards; retain them.
...
7. Keep the approved geometry invariant: exact retained line/arc middles, local six-control junction splines, and evenly spaced direct-control splines for bounded `<=6P` entity runs. Do not solve capacity or timing problems by reverting to a whole-path spline fit.
...
3. Profile and reduce whole-horizon planning work while the rolling planner is introduced; the 5,164-command `adaptive_pockets.ngc` window currently plans in about 2.81 seconds at P0.001 and 18.69 seconds at P0.002.
