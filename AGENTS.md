...
```powershell
cmake -S . -B build -G Ninja -DCMAKE_C_COMPILER=clang -DCMAKE_CXX_COMPILER=clang++ -DCMAKE_TOOLCHAIN_FILE="$env:VCPKG_ROOT\scripts\buildsystems\vcpkg.cmake"
```
...
Geometrically linear multi-knot splines retain one exact chord-distance inverse bracket per knot interval. Do not collapse them into one bracket across several knot intervals because spline arc-length inversion evaluates each bracket using one owning basis span.
...
Synchronization is explicit. Parameter reads, M6 preparation, externally visible `print`/`alert`, probing, homing, and M0 program pauses must not run ahead of required motion. `alert[...]` publishes a typed persistent operator message, while M0 is the sole continuation boundary and Resume is its authoritative acknowledgement. `GeometryStreamProducer` publishes synchronization/probe/program-pause fences and stops interpretation until the matching feedback arrives. Preview automatically resumes M0 after consuming prior geometry; timed execution releases it only after already-published motion reaches the required held boundary and the operator requests Resume. New operations that consume executor/hardware state or create externally ordered physical effects require an explicit barrier.
...
`BoundedLookaheadTrajectoryPlanner` collects compatible prepared G64 pieces across slice boundaries. `trajectory.lookahead_duration` is a minimum predicted duration for attempting a rolling prefix, not a hard horizon or semantic stop. Rolling splits require a dynamically feasible nonzero PVA boundary and a proved stop-feasible initial suffix section; the proof section is bounded near the lookahead duration and may end before the retained suffix so an unbounded cluster does not enter every feasibility probe. Rolling boundaries may use retained-line interiors or prepared cluster-spline knot boundaries; cluster splits partition the existing immutable curve, knot-interval timing metadata, and geometric samples without reconstructing geometry. A cluster-knot boundary velocity must respect both adjacent intervals' programmed feeds and prepared static geometric velocity caps, and both split sides must retain every source-command record referenced by their pieces. Do not enable arc-interior anchors by weakening exact boundary equivalence. Protected boundaries or `PreparedContinuousEnd` finalize the remaining chain.

PathTempo owns continuous scalar timing. NGC supplies ordered timing pieces with differential stations and boundary PVA, then materializes the scalar law into axis-space execution polynomials. Continuous timing must enforce programmed feed, aggregate path limits, per-axis limits, coupled acceleration `q' a + q'' v^2`, and coupled jerk `q' j + 3 q'' v a + q''' v^3`. PathTempo always applies its sampled differential-station corrections during the initial timing passes using the configured limits without caller-side derating. NGC then adaptively constructs local-coordinate endpoint-PVA quintics for all normal continuous motion, certifies their derivative-control bounds and ordered geometry, returns unresolved per-piece corrections to PathTempo in one batch, and emits the complete verified quintic sequence only after the corrected solve converges. NGC's materializer and final packet gates allow a one-percent verification tolerance above configured aggregate and per-axis acceleration and jerk limits; velocity uses only a `1e-7` relative numerical ratio tolerance. A sub-servo quintic that exceeds the tolerated pointwise jerk limit may pass only when its complete acceleration-control hull proves that the resulting acceleration excursion fits within the same tolerated servo-period jerk budget. NGC's emitted-polynomial extrema, C2 continuity, capacity, activation ownership, and stop safety remain final gates. Exact-stop normal motion and every stop tail remain cubics padded into the same degree-aware representation and use the same one-percent dynamic-limit verification tolerance. Bounded planning resource exhaustion is a detailed fatal error, not permission to publish partial or unproved work.

Production `PlanChunk` values carry a fixed-capacity ordered array of generic execution markers. Each marker contains only an opaque ID, normal-span ordinal, and normalized span parameter. `MotionBackend` emits the opaque marker event when its actual normal-trajectory cursor crosses that parameter, including every ordered marker crossed in one servo tick. Feed hold and resume preserve the marker cursor, so they neither emit a marker early nor repeat it. The NRT execution driver maps marker IDs to presentation state; G-code commands, presentation objects, and UI state never enter the backend. Marker capacity exhaustion is fatal planning evidence, not permission to activate early or drop an event. Production activation does not require polynomial splitting.

NGC must not duplicate PathTempo's differential-station reduction into caller-supplied per-piece acceleration or jerk limits. It supplies programmed feed, prepared static geometric velocity caps, differential stations, and machine-wide path/axis limits; PathTempo returns the effective per-piece limits used by NGC's materialization and diagnostics.

Continuous compilation stages verified NRT polynomials and packetizes them into `PlanChunk` values with at most 256 normal execution spans and 16 stop-tail spans. `AxisPolynomialSpan` is a bounded, trivially copyable degree-aware local-coordinate representation for padded cubics and quintics; the backend evaluates position, velocity, acceleration, and jerk directly from it without allocation. Geometry pieces may cross packet boundaries. Every packet must pass the stop-branch gate: the cubic tail starts from the immutable quintic branch state, is position/velocity/acceleration continuous, respects the numerical velocity tolerance and tolerated dynamic-limit verification, matches its declared terminal state, and ends stationary. Selecting a stop tail while an unpublished moving suffix exists is fatal until a proved resume path exists.
