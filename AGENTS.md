...
PathTempo owns continuous scalar timing. NGC supplies ordered timing pieces with differential stations and boundary PVA, then materializes PathTempo's time-domain cubics into axis-space execution spans. Continuous timing must enforce programmed feed, aggregate path limits, per-axis limits, coupled acceleration `q' a + q'' v^2`, and coupled jerk `q' j + 3 q'' v a + q''' v^3`. Materialized checking is the default and production-authoritative mode: NGC's exact emitted-polynomial extrema, ordered geometry verification, C2 continuity, capacity, activation ownership, and stop safety remain its final gates, and any exact materialization correction must be returned as tighter per-piece scalar limits and re-solved by PathTempo. The explicitly selected `Sampled` diagnostic mode instead uses PathTempo's differential-station constraint corrections. The explicitly selected `GeometryDiagnostic` mode plans with 99 percent of configured acceleration and jerk limits, returns corrections from NGC evaluations of the actual prepared-geometry differentials, and records exact emitted-cubic violations without using them as corrections. It also benchmarks an NRT-only adaptive endpoint-PVA quintic candidate using certified derivative-control bounds; those candidates and their failures are diagnostic evidence and must never enter `PlanChunk` or the backend. Neither diagnostic mode continuously proves emitted polynomial constraints, so their results are comparison data rather than production feasibility evidence; both retain NGC's geometry-tolerance, continuity, capacity, activation, and stop-safety gates. Bounded planning resource exhaustion is a detailed fatal error, not permission to publish partial or unproved work.

NGC must not duplicate PathTempo's differential-station reduction into caller-supplied per-piece acceleration or jerk limits. It supplies programmed feed, prepared static geometric velocity caps, differential stations, and machine-wide path/axis limits; PathTempo returns the effective per-piece limits used by NGC's materialization, cache, and diagnostics.
