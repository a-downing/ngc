...
namespace ngc::pendant::vista_cnc_p2s {
    namespace {
        ...
        std::string positionRow(double value) {
            ...
            else result.insert(result.begin(), 8 - result.size(), ' ');
            return result;
        }
        ...
        std::string compactSigned(double value, const std::size_t width) {
            ...
            else result.append(width - result.size(), ' ');
            return result;
        }
    }

    std::string formatPositionDisplay(const std::string_view workCoordinateSystem,
                                      const Axis axis, const double workPosition) {
        return heading(workCoordinateSystem, axis) + positionRow(workPosition);
    }


    std::string formatZeroDisplay(const std::string_view workCoordinateSystem,
                                  const Axis axis, const double workPosition,
                                  const double stagedWorkPosition) {
