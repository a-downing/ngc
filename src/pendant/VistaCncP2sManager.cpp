...
namespace ngc::pendant::vista_cnc_p2s {
    namespace {
        InputChange changesBetween(const InputState &previous, const InputState &current) noexcept {
            ...
            if(current.wheelDeltaValid && current.wheelDelta != 0) changes |= InputChange::Wheel;
            if(current.leftSelector != previous.leftSelector) changes |= InputChange::LeftSelector;
            if(current.rightSelector != previous.rightSelector) changes |= InputChange::RightSelector;
            if(current.wheelButton != previous.wheelButton) changes |= InputChange::WheelButton;
    ...
    Manager::Manager(std::unique_ptr<Driver> driver, const std::chrono::milliseconds reportTimeout)
        : m_driver(std::move(driver)), m_reportTimeout(reportTimeout) {
    ...
    std::expected<void, HidError> Manager::setDisplay(const std::string_view text) {
        {
            ...
            if(m_displayError) return std::unexpected(*m_displayError);
            const auto next = std::string(text.substr(0, 16));
            if(next == m_displayText) return {};
            m_displayText = next;
            ++m_displayRevision;
        }
