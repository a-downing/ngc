...
The earlier production-budget experiment stopped adaptive 100x at the per-pass geometry-verification bound rather than recursing into a 27.8 ns interval. The exporter now has explicit bounded offline correction and cumulative-geometry ceilings, leaving production defaults unchanged. Under those profiles, the current `adaptive.ngc` 10x and 100x horizons both complete their full emitted-polynomial proof. Adaptive 1000x remains outside the measured sweep. Dense 1000x was slightly slower than 100x in both historical models, so extreme jerk multipliers still expose numerical and representation limits rather than a guaranteed monotone asymptote.
...
### Direct infinite-jerk smoothed-path follow-up

`InfiniteJerkTrajectoryTime` now supplies an optimizer-free acceleration-only reference over the exact smoothed G64 pieces. It analytically intersects the scalar-acceleration interval implied by `q' a + q'' v^2`, propagates forward/backward squared-speed envelopes, and refines the travel-time integral. It is invoked only when the exporter passes an output pointer to `compileContinuous()`; normal planning does not pay for it. The result is a converged numerical comparison rather than an executable plan or formal lower-bound certificate.
...
The infinite-jerk duration is invariant across each jerk sweep because velocity, acceleration, feed, and geometry remain fixed. Its last-refinement delta was 0.000801955 seconds on `adaptive.ngc` and 0.000466037 seconds on `adaptive_pockets.ngc`. The 100x pocket run required 943,434 cumulative geometry-verification attempts but ultimately passed all exact extrema, geometry, C2, packetization, activation, and stop-branch gates. Explicit 10x exporter runs use 48 correction passes and 104 cumulative geometry attempts per piece; explicit 100x runs use the bounded maximum 128 and 256. Production remains at 24 and 36.
...
1. Use the implemented direct infinite-jerk envelope to locate acceleration-only loss on representative full-program horizons, and retain Clarabel only as an independent station-profile/jerk-aware experiment. Refine either numerical reference when its duration delta is too large for the comparison being claimed. Keep the fixed analytic line and arc-curvature regressions.
...
The cubic analyzer was subsequently extended from selected-spline experiments to a bounded corpus-wide `--optimize-all` mode. It now retains the captured line/arc primitives, trims the bounding primitives to the captured spline endpoints, and evaluates arcs through the production endpoint-exact `ArcReference`. Search uses a fixed ordered source-position proxy and a 100,000-candidate per-spline ceiling; every retained result is independently remeasured with the original high-resolution spline measurement and sampled normalized-arc-length association against production primitive positions. The report records path-length change, curvature and normal-curvature-derivative changes, source deviation, endpoint errors, candidate counts, budget exhaustion, rejection reason, and computation time. This remains diagnostic rather than a replacement for the planner's recursive ordered geometry proof.

On the 64-horizon `adaptive.ngc` rolling trace, 4,380 captured splines contained 236 variable-control short-entity splines; the other 4,144 were ordinary six-control junction blends and were deliberately excluded. The bounded search retained improvements for 73 of 236 short-entity splines, with zero source-deviation, curvature, endpoint, or objective regressions at the final diagnostic gate and no exhausted candidate budgets. Among retained candidates, sampled peak normal curvature derivative fell by 62.96% on average, the median theoretical local jerk-limited speed gain from the `kappa_s v^3` term was 1.378, and the maximum was 1.470. Total optimizer time was 34.48 seconds for 97,388 evaluated candidates in the Debug-runtime development build. The remaining 163 splines had no accepted coordinate-search update under this objective and bound; production integration must therefore preserve the original controls when the bounded search cannot prove a strict improvement.

A simpler construction experiment kept the existing first and last three endpoint controls, replaced the free controls with `ceil(interior_entity_count/2)` controls, and sampled them at equal arc-length bin midpoints directly on the captured interior primitive chain. It was inexpensive (the complete 236-spline analyzer pass took about two seconds), but it made the curvature-derivative spikes worse on `adaptive.ngc`: only 10 of 236 splines reduced sampled peak normal curvature derivative, while the median peak increased by 23.38%, the 90th percentile increased by 40.01%, and the worst case increased by 162.18%. Integrated normal curvature derivative improved on 45 splines but increased by 13.52% at the median. None of the 73 original eight-control splines improved; reducing their two free controls to one increased the median peak by 24.48%. Sampled source deviation increased on all 236 splines, by 81.85% at the median. Only 12 candidate maxima occurred exactly at a simple knot, so the regression is not primarily a mathematical C2 knot-jump artifact; the sparser control polygon changes the within-span geometry and endpoint-to-interior transition. This half-entity rule remains analyzer-only negative evidence.

For direct GUI inspection, preview and execution were temporarily changed to use `ceil(interior_entity_count/3)` equal arc-length bin-midpoint controls with no post-sampling fairing. The 64-horizon `adaptive.ngc` rolling plan completed without a proof failure, but planned duration increased from the preceding 494.035699186-second checkpoint to 598.030153497 seconds and packet count increased from 187 to 188. Two repeated regions dominated the regression: horizons 3 and 28 increased to 56.089804067 and 51.263757156 seconds. That one-third construction was subsequently replaced, without a build or timing measurement yet, by one directly sampled interior control per replaced entity for GUI evaluation.

With one direct control per replaced entity and one knot span per timing piece, the former proportional resource budgets stopped valid full-horizon proofs at their exact thresholds. `adaptive.ngc` required 8,680 geometry attempts against the former `3 * 2,893 = 8,679` per-pass bound. Raising the coefficient to four completed the full horizon with a measured per-pass high-water of 10,985 attempts under an 11,572 bound; planning took 2.184 seconds and produced a 307.532106312-second verified trajectory with 19,505 emitted spans. `adaptive_pockets.ngc` successively crossed the old 384- and experimental 512/768-per-curved-station cumulative candidate ceilings while each individual pass remained under its separate bound. A temporary maximum-pass probe measured convergence after nine correction replans at 5,531,119 evaluations across 6,408 curved stations, about 863 per station. The retained 1,024 coefficient gives an 18.6% reserve; the final full horizon planned in 7.177 seconds and produced a 305.274659685-second verified trajectory with 20,574 spans. Rolling trajectories remained structurally identical before and after the budget-only change: `adaptive.ngc` retained 58 horizons, 124 chunks, and 313.710883466 seconds; `adaptive_pockets.ngc` retained 54 horizons, 107 chunks, and 347.715038283 seconds.

These analyzers are not production geometry. Their primitive-deviation checks remain sampled; the cubic analyzer now queries the production adaptive `ArcReference`, while the quintic analyzer's standalone arc evaluator remains a directed endpoint-exact approximation. No experimental controls enter preview, the continuous compiler, `PlanChunk`, or `MotionBackend`. Before adopting either candidate, implement one shared preview/planner construction using the production arc reference, prove endpoint and ordered deviation bounds, preserve source feed/activation mapping, measure planning cost, and rerun the complete geometry, timing, capacity, stop-branch, rolling-boundary, and UI regressions.

## Certified inverse-cache profiling checkpoint (2026-07-16)

Windows Performance Recorder/Analyzer was tested against the symbolized Clang/Ninja oracle exporter. The final measurements use a separate Release build; the ordinary `build` directory links the debug C/C++ runtime and is useful for functional testing but distorts absolute profiling costs. WPR sampling plus NRT query counters showed that repeated spline and arc-length inversion dominated the first profile rather than Ruckig alone.

The former Ruckig submodule was first restored to clean official upstream `pantor/ruckig` at `2249d57` (`v0.17.3-8`) so the local delta could be audited. NGC now vendors only the fixed-DoF offline state-to-state subset under `src/third_party/ruckig`: 11 solver translation units, 12 reachable upstream headers, a small NGC-owned offline facade, and the verbatim MIT license. Cloud/waypoint support, the online update/output API, dynamic-DoF conveniences, wrappers, examples, tests, benchmarks, documentation, and upstream build packaging are omitted. A final-link map showed that all 11 core solver objects remain reachable because NGC uses position and velocity control, braking, first/second/third-order limits, disabled axes, and synchronized multi-axis solves. A controlled alternating Release A/B test found no measurable benefit from the former const-reference `get_profiles_ref()` extension: the 10-run means differed by about 0.005%, within noise. It was removed, and both phase-inspection sites use upstream's value-returning `get_profiles()` API.

The initial full-horizon `adaptive_pockets.ngc` Release counters were:

| Inverse kind | Queries | Adaptive inverse integrals | First-seed convergences |
| --- | ---: | ---: | ---: |
| Cubic B-spline | 8,724,852 | 11,979,293 | 4,791,408 |
| Endpoint-exact arc | 3,809,104 | 7,593,087 | 25,121 |

A monotone cubic Hermite spline-inverse seed built from the existing 32 nodes per knot span reduced inverse integrals by about 9.2%, but changed `adaptive_pockets.ngc` from 280.926324594 seconds and 14,307 verified spans to 281.534705422 seconds and 14,313 spans. That is unacceptable for a transparent optimization. The seed was removed rather than treating a numerically close inverse as semantically equivalent.

The accepted implementation uses a direct-mapped 16-entry cache per reference, keyed by the bit pattern of the requested arc-length distance. Only a parameter already returned by the original certified inverse path is cached. A cache miss still uses the preintegrated ordered bracket, the actual adaptive integral, and at most 12 safeguarded Newton steps. There is no tolerance match, unchecked interpolation, cross-geometry reuse, global mutable state, or RT state.

On the same full horizon after both caches:

| Inverse kind | Exact cache hits | Adaptive inverse integrals | Reduction |
| --- | ---: | ---: | ---: |
| Cubic B-spline | 7,377,859 | 1,616,340 | 86.5% |
| Endpoint-exact arc | 2,237,411 | 3,130,549 | 58.8% |

The dense line/arc fixture reduced arc inverse integrals by 65.7%; `1001.ngc` reduced them by 68.8%. All three cached exports were byte-identical to their uncached exports. `adaptive_pockets.ngc` retained 280.926324594 seconds, 14,307 verified spans, 852,831 reachability candidates, and 58,416 geometry attempts. No spline or arc inverse reached its iteration limit; measured maxima were three and two Newton iterations respectively.

Measured Release planning improved as follows:

| `adaptive_pockets.ngc` workload | Before inverse caches | After spline cache | After spline + arc caches |
| --- | ---: | ---: | ---: |
| Full horizon | about 5.40 s | 3.74-4.03 s | 3.01-3.08 s |
| 54-horizon rolling plan | about 12.69 s | 7.26-7.99 s | 6.23 s |

The rolling result retained 54 horizons, 103 chunks, 329.769361022 seconds of planned motion, 241 boundary candidates, 183 suffix failures, and five prefix failures. Its published plans reported 2,101,980 exact arc-cache hits and 5,024,557 exact spline-cache hits. These diagnostics are NRT-only and never enter `MotionBackend`.

The final WPR profile moved the dominant work to local timing. Relative to `compileContinuous()` samples, `timeLawBetween()` was about 43% and Ruckig target calculation about 22%; arc inversion fell from roughly 30% before its cache to 15.7%, and spline inversion had already fallen from roughly 40% before its cache to about 8.5%. This motivated the subsequently completed analytic acceleration/jerk endpoint-interval experiment and Ruckig-call-purpose counters. Neither can replace transition solves, emitted-polynomial extrema, ordered geometry proof, or stop-branch verification.

Post-vendoring Release validation retained the full-horizon `adaptive_pockets.ngc` result exactly: 280.9263245941939 seconds, 14,307 verified spans, 852,831 reachability candidates, and 58,416 geometry attempts. The regenerated 97,696-segment oracle model had the same SHA-256 hash as the pre-vendoring cached model. A measured full-horizon calculation was 3.58 seconds. The rolling result also retained 54 horizons, 103 chunks, 329.7693610220958 seconds, 241 boundary candidates, 183 suffix failures, five prefix failures, and identical inverse-cache counters. Rolling calculations measured 7.55 and 8.90 seconds during the validation session versus the earlier 6.23-second checkpoint, so wall time remains subject to system-load variation; trajectory structure and proof diagnostics did not change.

The vendored solver audit found no simple source-level win in the current hot path. NGC already reuses its one-DoF input, generator, and trajectory workspace, and profiling places Ruckig's cost in required third-order quartic/root solving and candidate-profile verification rather than allocation or profile copying. Ruckig-only `-O3` was tested against the retained `-O2` binary in alternating order; it preserved the oracle hash but did not improve wall time and was frequently slower under the same system load, so `-O2` remains. Skipping validation, weakening floating-point semantics, approximating roots, or pruning candidate profiles without a proof would trade away failure detection or numerical correctness and was not adopted. The next useful optimization remains reducing the number of local solves with measured analytic endpoint filters, outside Ruckig's root algebra.

## Scalar time-law call instrumentation checkpoint (2026-07-16)

Every `timeLawBetween()` invocation now records NRT-only logical calls, successes, failures, actual solver calls, measured wrapper seconds, correction-pass calls, cache hits/misses/collisions, and selected-hit materializations. Primary categories are exact-stop timing, continuous seed timing, current-velocity station candidates, station-cap probes, and binary velocity-bracket probes. `BoundedLookaheadTrajectoryPlanner` separately aggregates all attempts, published horizons, prefix probes, and suffix probes, including failed rolling candidates.

The Release full-horizon `adaptive_pockets.ngc` measurement was:

| Category | Calls | Successes | Failures | Exact repeats | Correction-pass calls | Measured seconds |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| Continuous seed | 24,424 | 24,424 | 0 | 17,156 | 21,371 | 0.018 |
| Current velocity | 178,109 | 130,665 | 47,444 | 134,186 | 154,585 | 0.189 |
| Station cap | 147,453 | 16,090 | 131,363 | 114,817 | 130,978 | 0.220 |
| Binary bracket | 846,103 | 281,929 | 564,174 | 603,701 | 734,674 | 1.174 |
| **Total** | **1,196,089** | **453,108** | **742,981** | **869,860 (72.7%)** | **1,041,608 (87.1%)** | **1.601** |

The instrumented full export retained 280.9263245941939 seconds, 14,307 verified spans, 852,831 reachability candidates, and 58,416 geometry attempts. Its model SHA-256 remained `CA41770AD49C184156C1B4F915455931B139873095F4855166FA74545D4CC2C4`.

The production-effort rolling run made 2,002,238 calls across all attempts, with 1,204,804 bit-exact repeats (60.2%) inside individual compile operations. Published horizons accounted for 968,982 calls; prefix probes made 955,420 calls and suffix probes made 1,018,986.

The initially compile-local direct-mapped cache stores the exact eight-value key, validated feasibility, and validated duration. It deliberately does not store a full fixed-capacity `TimeLaw`: that would multiply the table footprint by hundreds of bytes per entry. A failure hit rejects the identical candidate. A success hit participates in duration comparison, but a winning hit is rematerialized by Ruckig and passes the complete monotonicity validation before its phases are retained. Seed and exact-stop calls continue through the full solver path. Only its bounded storage lifetime was later extended across compilations.

Small horizons use 32,768 entries; horizons with more than 1,024 geometry pieces use 131,072. On the full workload, the large table recorded 642,213 hits and reduced actual solver executions from 1,196,089 to 589,679, including 35,803 rematerializations. Instrumented Release planning fell from 3.99 seconds to 2.45-2.60 seconds. A 262,144-entry experiment reduced solver calls further to 511,972 but produced the same 2.45-second wall time, so its doubled initialization and memory footprint were rejected.

Adaptive sizing reduced the 54-horizon rolling calculation from 6.80 seconds to 5.00 seconds. Across all rolling attempts it recorded 1,133,537 hits and reduced actual solver executions from 2,002,238 to 920,136, including 51,435 rematerializations. Relative to the best earlier pre-instrumentation checkpoints, the retained implementation is approximately 16-20% faster on the full horizon and 20% faster in rolling mode. The final full model remained byte-identical at SHA-256 `CA41770AD49C184156C1B4F915455931B139873095F4855166FA74545D4CC2C4`.

## Coupled endpoint experiment and retained geometry reuse (2026-07-16)

The endpoint candidate check previously called `sampleAt()`, `curvatureAt()`, and `curvatureDerivativeAt()` for both one-sided station geometries on every acceleration candidate and correction pass. Those values depend only on the fixed geometry piece. The retained implementation now computes the exact start/end `T`, `kappa`, and `kappa_s` values once per piece, reuses them in the unchanged coupled acceleration/jerk authority check, and reports cached endpoint count, candidate checks, acceleration rejections, and jerk rejections as NRT diagnostics.

On the full `adaptive_pockets.ngc` workload, 6,106 cached one-sided endpoints served 852,831 candidate checks. The check rejected zero candidates for acceleration and 19,154 for jerk before local timing. Spline inverse queries fell to 3,750,131 and arc inverse queries to 884,094. Release planning measured 1.88-1.92 seconds, down from the 2.45-2.60-second timing-cache checkpoint. The 54-horizon rolling calculation measured 3.76 seconds, down from 5.00 seconds, while retaining 54 horizons, 103 chunks, 329.7693610220958 seconds, 241 boundary candidates, 183 suffix failures, five prefix failures, and all timing-cache counts.

An explicit rejection-only acceleration/existence-of-jerk projection was implemented and tested in an enabled/disabled Release A/B. It evaluated 191,834 full-horizon velocity candidate sets, found no empty acceleration or jerk interval, and preclassified the same 19,154 jerk failures already caught by the cached-geometry authority check. Actual Ruckig calls remained exactly 589,679 full-horizon and 920,136 rolling. The projection increased planning to 1.92 seconds full-horizon and 4.09 seconds rolling versus 1.88 and 3.76 seconds with endpoint caching alone, so it was removed. Its formulas remain relevant to a future backward reachable-set representation, but are not a profitable standalone filter for the current lattice.

The final 97,696-segment model still contains 5,164 motions, 14,307 verified spans, 852,831 reachability candidates, 58,416 geometry attempts, and duration 280.9263245941939 seconds. Its SHA-256 remains `CA41770AD49C184156C1B4F915455931B139873095F4855166FA74545D4CC2C4`.

The final extrema tolerance remains strict after a rejected 1% acceleration/jerk allowance experiment. Applying a true 1% measured-limit allowance consistently to both continuous correction and final publication shortened full motion by only 0.090% and rolling motion by 0.063%. It increased full planning from 1.917 to 2.056 seconds, added one correction pass, raised candidate evaluations from 852,831 to 956,187, and increased verified spans from 14,307 to 14,322. Rolling planning increased from 3.755 to 3.983 seconds and packet count from 103 to 104. The experiment was removed; do not reinterpret `1e-9` as a physical headroom policy.

## Correction-pass locality checkpoint (2026-07-16)

Every correction replan now records an NRT-only bit-exact comparison with the preceding pass. Station velocity and acceleration are compared by representation, and a piece timing is reusable only when its complete scalar phase-boundary sequence—including time, distance, velocity, acceleration, outgoing jerk, and brake-phase identity—is identical. Diagnostics also retain corrected-piece and changed-timing runs, exact reusable runs and edge regions, maximum numerical changes, and propagation distance from the nearest corrected piece. The counters aggregate separately for all rolling attempts, published horizons, prefix probes, and suffix probes.

The full 3,053-piece `adaptive_pockets.ngc` horizon required seven replans. Across 21,371 pass-to-pass piece comparisons, 4,730 timings changed and 16,641 (77.9%) remained exact. Changes formed 1,135 islands; their maximum contiguous length was 37 pieces and their maximum distance from a corrected piece was six pieces. Early corrected pieces were distributed from nearly the beginning to the end of the horizon, so replanning one interval between the first and last correction would save little. The final replan changed only five timings while 3,048 remained exact.

All rolling attempts made 457 replans and compared 32,746 timings. Of those, 23,097 (70.5%) remained exact; the maximum changed run was 29 pieces and maximum propagation from the nearest corrected piece was seven. Published horizons retained 11,438 of 16,269 timings (70.3%). Prefix and suffix probes independently showed 70.7% and 70.8% exact reuse. These measurements support an exact-keyed multi-island worklist or station-visit replay, not a guessed fixed halo. Reuse must require matching complete local inputs and retained neighboring timings and must leave exact polynomial and publication verification authoritative.

The diagnostic build retained 280.9263245941939 seconds, 14,307 spans, 852,831 candidates, 58,416 geometry attempts, 54 rolling horizons, 103 chunks, and 329.7693610220958 rolling seconds. Its oracle SHA-256 remained `CA41770AD49C184156C1B4F915455931B139873095F4855166FA74545D4CC2C4`. Debug and Release tests passed.

The follow-up station-visit shadow mode uses the same station/sweep/direction slot in the preceding correction pass and compares a collision-free exact serialization of all local scalar inputs and adjacent phase boundaries. It does not skip work. Instead, it executes the visit and accepts a replay opportunity only when the output station state and both adjacent `TimeLaw` sequences are also bit-identical. It is disabled by default and exposed to the exporter as `--effort=replay-shadow`; suffix probes now inherit explicit effort profiles just like prefix planners.

On the full horizon, 17,368 of 27,089 comparable active visits matched exactly. All 17,368 outputs matched, with zero mismatches. These visits consumed 453,274 candidate checks, 628,532 logical timing calls, 212,647 actual Ruckig calls, 33,341 materializations, and approximately 0.51 seconds. The logical calls are 60.3% of all correction-pass calls and the solver calls are 36.1% of all full-horizon solver executions.

Across all rolling attempts, 13,879 of 29,022 comparable visits matched and every output matched. Their gross work was 488,148 candidate checks, 690,919 timing calls, 80,887 solver calls, 46,370 materializations, and approximately 0.19 seconds. This is 43.9% of correction calls but only 8.8% of actual rolling solver calls because the retained timing cache already absorbs most exact repeats. A real replay prototype is justified, but it must replace the diagnostic vectors with compact fixed-layout snapshots/direct comparison and must reproduce logical candidate and resource-budget counters. Shadow mode itself is too expensive to enable during ordinary planning.

The first opt-in `--effort=replay` prototype performed the real skip with the shadow mode's exact serialized key and retained output phases. It restores the station state and adjacent `TimeLaw` values and advances every logical candidate, endpoint-rejection, velocity-search, and cumulative resource-budget counter by the recorded visit deltas. Full timing calls fell from 1,196,089 to 567,557 and Ruckig executions from 589,679 to 373,819. Rolling timing calls fell from 2,002,238 to 1,311,319 and solver executions from 920,136 to 838,465. All trajectory, proof-work, and rolling-topology results remained identical.

The final retained representation replaces both dynamically allocated serialized vectors with a fixed 160-word input key and compares outputs directly through the retained station state and adjacent `TimeLaw` values. A paired Release run improved full planning from 1.979 to 1.513 seconds (23.6%) and rolling from 4.201 to 3.981 seconds (5.2%). Compact replay is now enabled in the production effort defaults. The exporter retains `--effort=no-replay` for controlled comparisons, explicit `--effort=replay` as an equivalent named profile, and `--effort=replay-shadow` for execute-and-compare validation. Replay is disabled automatically when an offline effort profile requests more than three station sweeps, bounding retained NRT visit memory.

## Cross-compilation timing-cache checkpoint (2026-07-16)

A planner-local prepared-geometry prototype improved alternating rolling runs by only 3.45% and was removed to avoid its exact command key, cached-object lifetime, inverse-diagnostic deltas, and changed suffix-planner ownership.

The simpler retained change shares the exact time-law result cache across all compilations and planners on one planning thread. It is a bounded 128K thread-local direct-mapped table; small rolling horizons retain the measured 32K index range. Thread-local ownership avoids locks and data races between workers. The existing eight-value key fully identifies the scalar solve, and selected success hits still rerun Ruckig to materialize validated phase data.

In two alternating rolling-only pairs, compile-local caches measured 3.830 and 3.827 seconds while the shared cache measured 3.636 and 3.583 seconds. Means were 3.829 versus 3.609 seconds, a 5.74% improvement. Actual solver calls fell from 838,465 to 648,024 (22.7%), cache hits rose from 477,956 to 700,107, and selected-hit materializations rose from 5,102 to 36,812. All logical work and trajectory structure remained identical. A full-plus-rolling validation retained SHA-256 `CA41770AD49C184156C1B4F915455931B139873095F4855166FA74545D4CC2C4`, 54 horizons, 103 chunks, 329.7693610220958 rolling seconds, 241 boundary candidates, 183 suffix failures, and five prefix failures. The shared cache is the default; `--effort=local-time-cache` is the A/B control.
